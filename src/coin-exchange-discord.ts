...
import {
  ...
  addUserToExchangeAllowlist,
  grantCoins,
  isExchangeUserAllowed,
  listExchangeAllowlist,
  listUserOffers,
  listUserTransactions,
  parseCoinAmount,
  removeUserFromExchangeAllowlist,
  transferCoins,
} from './coin-exchange';
...
function resolveServerScope(serverId?: string | null) {
  return serverId ?? 'global';
}

async function canUseExchange(serverId: string, userId: string, adminIds: Set<string>) {
  if (adminIds.has(userId)) return true;
  return isExchangeUserAllowed(serverId, userId);
}
...
function buildSectionDivider(title: string) {
  return `──── ${title} ────`;
}

function buildMetricGrid(items: Array<{ label: string; value: string }>) {
  return items.map((item) => `• ${item.label}: **${item.value}**`).join('\n');
}

function buildOfferPreviewRows(limitOffers: Awaited<ReturnType<typeof listCoinOffers>>) {
...
function buildHubButtons() {
  const rows: Array<ActionRowBuilder<ButtonBuilder>> = [
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId('exchange_hub_open').setLabel('Open Exchange').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId('exchange_hub_profile').setLabel('My Profile').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId('exchange_hub_market').setLabel('Market').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId('exchange_hub_status').setLabel('Status').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId('exchange_hub_refresh').setLabel('Refresh Boards').setStyle(ButtonStyle.Secondary),
    ),
  ];
  ...
  if (EXCHANGE_WEB_URL) {
    rows.push(
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setStyle(ButtonStyle.Link).setURL(EXCHANGE_WEB_URL).setLabel('Open Full-Screen Exchange')
      )
    );
  }
...
async function buildHubEmbeds() {
  ...
  const [overview, openOffers, recentTransactions, leaders] = await Promise.all([
    getCoinExchangeOverview(serverId),
    listCoinOffers(8, 'open', serverId),
    listCoinTransactions(8, '', serverId),
    listCoinWallets(10, serverId),
  ]);
  ...
  const traderPortal = new EmbedBuilder()
...
function getHubFooterKey(message: Message) {
  return message.embeds[0]?.footer?.text ?? '';
}

async function findExistingHubMessages(channel: TextChannel) {
  const pinned = await channel.messages.fetchPins().catch(() => null);
  const source: Array<Message<true>> = [];
  if (pinned) {
    const anyPinned = pinned as any;
    if (Array.isArray(anyPinned)) {
      source.push(...(anyPinned as Array<Message<true>>));
    } else if (typeof anyPinned.values === 'function') {
      source.push(...Array.from(anyPinned.values() as Iterable<Message<true>>));
    } else if (typeof anyPinned[Symbol.iterator] === 'function') {
      source.push(...Array.from(anyPinned as Iterable<Message<true>>));
    }
  }
...
async function cleanupLegacyHubMessages(client: Client) {
  ...
  for (const channelId of channelIds) {
    ...
    const hubMessages = await findExistingHubMessages(channel as TextChannel);
...
async function buildMainPanelEmbed(user: User, serverId: string) {
  const [wallet, overview, recentTx, openOffers] = await Promise.all([
    getCoinWallet(user.id, serverId),
    getCoinExchangeOverview(serverId),
    listUserTransactions(user.id, 3, serverId),
    listCoinOffers(6, 'open', serverId),
  ]);
...
function buildBalanceBar(available: number, total: number): string {
  if (total === 0) return '`░░░░░░░░░░` 0%';
...
async function buildMarketEmbed(userId: string, page: number, pageSize: number, serverId: string) {
  const allOffers = await listCoinOffers(50, 'open', serverId);
...
async function buildLeaderboardEmbed(serverId: string) {
  const wallets = await listCoinWallets(10, serverId);
  const medals = ['🥇', '🥈', '🥉'];
...
async function buildProfileEmbed(user: User, serverId: string) {
  const [wallet, leaderboard] = await Promise.all([
    getCoinWallet(user.id, serverId),
    listCoinWallets(100, serverId),
  ]);
...
async function buildStatusEmbed(serverId: string) {
  const [overview, recentTransactions, recentOffers] = await Promise.all([
    getCoinExchangeOverview(serverId),
    listCoinTransactions(150, '', serverId),
    listCoinOffers(150, '', serverId),
  ]);
...
function buildMainButtons(userId: string) {
  const rows: Array<ActionRowBuilder<ButtonBuilder>> = [
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      ...
      new ButtonBuilder().setCustomId(`exchange_market:${userId}:1`).setLabel('📊 Live Market').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId(`exchange_offers:${userId}:1`).setLabel('📨 Offer Desk').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`exchange_history:${userId}:1`).setLabel('🧾 History').setStyle(ButtonStyle.Secondary)
    ),
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`exchange_send_modal:${userId}`).setLabel('➡️ Send').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`exchange_offer_modal:${userId}`).setLabel('📝 Create').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId(`exchange_accept_modal:${userId}`).setLabel('✅ Accept').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`exchange_cancel_modal:${userId}`).setLabel('🛑 Cancel').setStyle(ButtonStyle.Danger)
    ),
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`exchange_profile:${userId}`).setLabel('👤 Profile').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`exchange_status:${userId}`).setLabel('📈 Status').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`exchange_leaderboard:${userId}`).setLabel('🏆 Top').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId(`exchange_help:${userId}`).setLabel('❓ Help').setStyle(ButtonStyle.Secondary)
    ),
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`exchange_refresh:${userId}`).setLabel('🔄 Refresh Dashboard').setStyle(ButtonStyle.Secondary)
    ),
  ];
...
function buildBackButtons(userId: string) {
  return [
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`exchange_send_modal:${userId}`).setLabel('➡️ Send Coins').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`exchange_offer_modal:${userId}`).setLabel('📝 New Offer').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId(`exchange_back:${userId}`).setLabel('🏠 Home').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`exchange_refresh:${userId}`).setLabel('🔄 Refresh').setStyle(ButtonStyle.Secondary)
    ),
  ];
}

function buildPagedButtons(userId: string, action: 'exchange_offers' | 'exchange_history' | 'exchange_market', page: number, totalPages: number) {
...
function buildOfferIdModal(userId: string, mode: 'accept' | 'cancel') {
  const modal = new ModalBuilder()
    .setCustomId(`exchange_${mode}_submit:${userId}`)
    .setTitle(mode === 'accept' ? 'Accept Offer' : 'Cancel Offer');

  const offerIdInput = new TextInputBuilder()
    .setCustomId('offerId')
    .setLabel('Offer ID')
  ...
  modal.addComponents(new ActionRowBuilder<TextInputBuilder>().addComponents(offerIdInput));
  return modal;
}
...
async function handlePanelButton(interaction: ButtonInteraction) {
  ...
  if (action === 'exchange_send_modal') {
    ...
    if (!recipientUserId || amount === null) {
      await submit.reply({ content: 'Invalid recipient or amount.', flags: MessageFlags.Ephemeral });
      return;
    }
    ...
    await submit.reply({ content: `Transferred **${amount.toFixed(2)}** coins to <@${recipientUserId}>.`, flags: MessageFlags.Ephemeral });
    return;
  }
  ...
  if (action === 'exchange_offer_modal') {
    ...
    await submit.reply({ content: `Created offer **#${offerId}** for <@${recipientUserId}> worth **${amount.toFixed(2)}** coins.`, flags: MessageFlags.Ephemeral });
    return;
  }
  ...
  if (action === 'exchange_accept_modal' || action === 'exchange_cancel_modal') {
    ...
    if (!Number.isInteger(offerId) || offerId < 1) {
      await submit.reply({ content: 'Invalid offer id.', flags: MessageFlags.Ephemeral });
      return;
    }

    if (mode === 'accept') {
      await acceptExchangeOffer({ recipientUserId: submit.user.id, offerId, serverId: submit.guild?.id ?? 'dm' });
      await submit.reply({ content: `Accepted offer **#${offerId}**.`, flags: MessageFlags.Ephemeral });
    } else {
      await cancelExchangeOffer({ requesterUserId: submit.user.id, offerId, serverId: submit.guild?.id ?? 'dm' });
      await submit.reply({ content: `Cancelled offer **#${offerId}**.`, flags: MessageFlags.Ephemeral });
    }
    return;
  }

  if (action === 'exchange_wallet') {
    const wallet = await getCoinWallet(interaction.user.id, serverId);
  ...
  if (action === 'exchange_offers') {
    const offers = await listUserOffers(interaction.user.id, serverId);
    const pageSize = 3;
  ...
  if (action === 'exchange_offer_accept' || action === 'exchange_offer_cancel') {
    ...
    if (!Number.isInteger(offerId) || offerId < 1) {
      await interaction.reply({ content: 'Invalid offer id.', flags: MessageFlags.Ephemeral });
      return;
    }
  ...
  if (action === 'exchange_history') {
    const transactions = await listUserTransactions(interaction.user.id, 24, serverId);
    const pageSize = 6;
  ...
  await interaction.reply({ content: 'Unsupported dashboard action.', flags: MessageFlags.Ephemeral });
}
...
async function publishExchangeHub(channel: TextChannel) {
...
async function refreshAllExchangeHubs(client: Client) {
  const channelIds = (process.env.COIN_EXCHANGE_CHANNEL_IDS ?? process.env.COIN_EXCHANGE_CHANNEL_ID ?? DEFAULT_CHANNEL_ID)
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean);
  ...
  for (const channelId of channelIds) {
    const channel = await client.channels.fetch(channelId).catch(() => null);
    if (!channel || !channel.isTextBased() || channel.isDMBased()) continue;
    await publishExchangeHub(channel as TextChannel).catch(() => undefined);
  }
}
...
async function registerExchangeSlashCommands(client: Client, guildIds: string[]) {
  const token = process.env.DISCORD_TOKEN;
  const clientId = process.env.DISCORD_CLIENT_ID;
  if (!token || !clientId || !guildIds.length) return;
...
async function handleExchangeSlashCommand(interaction: ChatInputCommandInteraction, adminIds: Set<string>) {
  ...
  if (subcommand.startsWith('allow-')) {
    if (!adminIds.has(interaction.user.id)) {
      await interaction.reply({ content: 'Only exchange admins can manage the allowlist.', flags: MessageFlags.Ephemeral });
      return;
    }

    if (subcommand === 'allow-list') {
      const entries = await listExchangeAllowlist(serverId, 200);
    ...
    if (subcommand === 'allow-add') {
      const added = await addUserToExchangeAllowlist({ serverId, userId: target.id, addedByUserId: interaction.user.id });
      await interaction.reply({ content: added ? `Added <@${target.id}> to exchange allowlist.` : `<@${target.id}> is already allowlisted.`, flags: MessageFlags.Ephemeral });
      return;
    }

    if (subcommand === 'allow-remove') {
      const removed = await removeUserFromExchangeAllowlist({ serverId, userId: target.id });
      await interaction.reply({ content: removed ? `Removed <@${target.id}> from exchange allowlist.` : `<@${target.id}> was not in the allowlist.`, flags: MessageFlags.Ephemeral });
      return;
    }
  }
  ...
  if (!allowed) {
    await interaction.reply({ content: 'Exchange access is restricted. Ask an admin to add you to the allowlist.', flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'open') {
    const embed = await buildMainPanelEmbed(interaction.user, serverId);
    await interaction.reply({ embeds: [embed], components: buildMainButtons(interaction.user.id), flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'wallet') {
    const wallet = await getCoinWallet(interaction.user.id, serverId);
    await interaction.reply({ embeds: [buildWalletEmbed(interaction.user, wallet)], flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'market') {
    const { embed, page, totalPages } = await buildMarketEmbed(interaction.user.id, 1, 6, serverId);
    await interaction.reply({ embeds: [embed], components: buildPagedButtons(interaction.user.id, 'exchange_market', page, totalPages), flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'top') {
    await interaction.reply({ embeds: [await buildLeaderboardEmbed(serverId)], flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'profile') {
    await interaction.reply({ embeds: [await buildProfileEmbed(interaction.user, serverId)], flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'status') {
    await interaction.reply({ embeds: [await buildStatusEmbed(serverId)], flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'history') {
    const tx = await listUserTransactions(interaction.user.id, 24, serverId);
    await interaction.reply({ embeds: [buildHistoryEmbed(tx, 1, 6, Math.max(1, Math.ceil(tx.length / 6)))], flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'hub') {
    if (!adminIds.has(interaction.user.id)) {
      await interaction.reply({ content: 'You do not have permission to publish the exchange hub.', flags: MessageFlags.Ephemeral });
      return;
    }

    const channel = interaction.channel;
    if (!channel || !channel.isTextBased() || channel.isDMBased()) {
      await interaction.reply({ content: 'Hub publishing requires a guild text channel.', flags: MessageFlags.Ephemeral });
      return;
    }

    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    await publishExchangeHub(channel as TextChannel);
  ...
  if (subcommand === 'send') {
    ...
    const amount = parseCoinAmount(String(interaction.options.getNumber('amount', true)));
    if (amount === null) {
      await interaction.reply({ content: 'Invalid amount.', flags: MessageFlags.Ephemeral });
      return;
    }

    await transferCoins({ fromUserId: interaction.user.id, toUserId: user.id, amount, serverId: interaction.guildId ?? 'dm' });
    await interaction.reply({ content: `Transferred **${amount.toFixed(2)}** coins to <@${user.id}>.`, flags: MessageFlags.Ephemeral });
    return;
  }
  ...
  if (subcommand === 'offer') {
    ...
    const offerId = await createExchangeOffer({ senderUserId: interaction.user.id, recipientUserId: user.id, amount, note, serverId: interaction.guildId ?? 'dm' });
    await interaction.reply({ content: `Created offer **#${offerId}** for <@${user.id}> worth **${amount.toFixed(2)}** coins.`, flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'accept') {
    const offerId = interaction.options.getInteger('offer_id', true);
    await acceptExchangeOffer({ recipientUserId: interaction.user.id, offerId, serverId: interaction.guildId ?? 'dm' });
    await interaction.reply({ content: `Accepted offer **#${offerId}**.`, flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'cancel') {
    const offerId = interaction.options.getInteger('offer_id', true);
    await cancelExchangeOffer({ requesterUserId: interaction.user.id, offerId, serverId: interaction.guildId ?? 'dm' });
    await interaction.reply({ content: `Cancelled offer **#${offerId}**.`, flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === 'grant') {
    if (!adminIds.has(interaction.user.id)) {
      await interaction.reply({ content: 'You do not have permission to grant exchange coins.', flags: MessageFlags.Ephemeral });
      return;
    }
    const user = interaction.options.getUser('user', true);
    const amount = parseCoinAmount(String(interaction.options.getNumber('amount', true)));
    ...
    const wallet = await grantCoins({ actorUserId: interaction.user.id, targetUserId: user.id, amount, serverId: interaction.guildId ?? 'dm', details: `Granted by ${interaction.user.tag}` });
    await interaction.reply({ content: `Granted **${amount.toFixed(2)}** coins to <@${user.id}>. New balance: **${wallet.availableBalance.toFixed(2)}**.`, flags: MessageFlags.Ephemeral });
  }
}

async function handleHubButton(interaction: ButtonInteraction, adminIds: Set<string>) {
  if (interaction.replied || interaction.deferred) return;
  const serverId = resolveServerScope(interaction.guild?.id);
  ...
  if (interaction.customId === 'exchange_hub_refresh') {
    if (!adminIds.has(interaction.user.id)) {
      await interaction.reply({ content: 'Only exchange admins can refresh the hub boards.', flags: MessageFlags.Ephemeral });
      return;
    }
    const channel = interaction.channel;
    if (!channel || !channel.isTextBased() || channel.isDMBased()) {
      await interaction.reply({ content: 'Hub refresh requires a guild text channel.', flags: MessageFlags.Ephemeral });
      return;
    }
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    await publishExchangeHub(channel as TextChannel);
...
export function registerCoinExchangeCommands(client: Client, ownerId: string) {
  ...
  client.on('interactionCreate', async (interaction: Interaction) => {
    try {
      ...
      if (interaction.isRepliable() && !interaction.replied && !interaction.deferred) {
        await interaction.reply({ content: `Coin exchange error: ${messageText}`, flags: MessageFlags.Ephemeral });
      }
    }
  });
  ...
  client.on('messageCreate', async (message: Message) => {
    ...
    if (action === 'allow') {
      ...
      if (mode === 'add') {
        const added = await addUserToExchangeAllowlist({ serverId, userId: targetUser.id, addedByUserId: message.author.id });
        await message.reply(added ? `Added <@${targetUser.id}> to exchange allowlist.` : `<@${targetUser.id}> is already allowlisted.`);
        return;
      }

      if (mode === 'remove') {
        const removed = await removeUserFromExchangeAllowlist({ serverId, userId: targetUser.id });
    ...
    try {
      ...
      if (action === 'wallet') {
        const wallet = await getCoinWallet(message.author.id, resolveServerScope(message.guild?.id));
      ...
      if (action === 'offers') {
        const offers = await listUserOffers(message.author.id, resolveServerScope(message.guild?.id));
      ...
      if (action === 'history') {
        const transactions = await listUserTransactions(message.author.id, 8, resolveServerScope(message.guild?.id));
