...
export function registerAffiliateCommands(client: Client, ownerId: string) {
  client.on('messageCreate', async (message: Message) => {
    ...
    if (message.content === '!affiliate request') {
      try {
        ...
        const [rows] = await db.execute(
          'SELECT status FROM affiliates WHERE user_id = ?',
          [message.author.id]
        );
        await db.end();
        ...
        await db2.execute(
          'INSERT INTO affiliates (user_id, status) VALUES (?, \'pending\') ON DUPLICATE KEY UPDATE status = \'pending\', requested_at = CURRENT_TIMESTAMP',
          [message.author.id]
        );
        await db2.end();
    ...
    if (message.content.startsWith('!affiliate approve ') && message.author.id === ownerId) {
      ...
      try {
        const db = await getDbConnection();
        const [rows] = await db.execute(
          'SELECT status FROM affiliates WHERE user_id = ? AND status = \'pending\'',
          [userId]
        );
        ...
        await db.execute(
          'UPDATE affiliates SET status = \'active\', approved_at = CURRENT_TIMESTAMP, approved_by = ? WHERE user_id = ?',
          [ownerId, userId]
        );
        await db.end();
    ...
    if (message.content.startsWith('!affiliate remove ') && message.author.id === ownerId) {
      ...
      try {
        const db = await getDbConnection();
        const [rows] = await db.execute(
          'SELECT status FROM affiliates WHERE user_id = ? AND status = \'active\'',
          [userId]
        );
        if ((rows as any[]).length === 0) {
          await db.end();
        ...
        await db.execute(
          'UPDATE affiliates SET status = \'removed\' WHERE user_id = ?',
          [userId]
        );
        await db.end();
    ...
    if (message.content === '!affiliate list' && message.author.id === ownerId) {
      try {
        const db = await getDbConnection();
        const [rows] = await db.execute(
          'SELECT user_id FROM affiliates WHERE status = \'active\''
        );
        await db.end();
    ...
    if (message.content === '!affiliate link') {
      try {
        const db = await getDbConnection();
        const [rows] = await db.execute(
          'SELECT user_id FROM affiliates WHERE user_id = ? AND status = \'active\'',
          [message.author.id]
        );
        await db.end();
