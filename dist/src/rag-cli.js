...
async function main() {
    ...
    if (args.includes('--index')) {
        const stats = await (0, rag_1.getRagIndex)(true);
    ...
    const summary = await (0, rag_1.answerWithContext)(query, Number.isFinite(topK) ? topK : 5);
    console.log(summary.answer);
    const details = await (0, rag_1.queryRag)(query, Number.isFinite(topK) ? topK : 5);
    console.log('\nContext snippets:\n');
