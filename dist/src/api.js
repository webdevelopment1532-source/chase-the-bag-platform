...
exports.app.get('/api/exchange/overview', sensitiveLimiter, async (req, res) => {
    try {
        ...
        const overview = await (0, coin_exchange_1.getCoinExchangeOverview)(serverId);
        res.json(overview);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
exports.app.get('/api/exchange/wallets', sensitiveLimiter, async (req, res) => {
    try {
        ...
        const limit = getLimit(req.query.limit, 50, 500);
        const serverId = getServerScope(req.query.serverId);
        const wallets = await (0, coin_exchange_1.listCoinWallets)(limit, serverId);
        res.json(wallets);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
exports.app.get('/api/exchange/offers', sensitiveLimiter, async (req, res) => {
    try {
        ...
        const offers = await (0, coin_exchange_1.listCoinOffers)(limit, status, serverId);
        res.json(offers);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
exports.app.get('/api/exchange/transactions', sensitiveLimiter, async (req, res) => {
    try {
        ...
        const transactions = await (0, coin_exchange_1.listCoinTransactions)(limit, userId, serverId);
