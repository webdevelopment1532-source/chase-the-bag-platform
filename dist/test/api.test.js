...
describe('api helpers and routes', () => {
    ...
    async function loadApiModule(overrides) {
        ...
        const api = await Promise.resolve().then(() => __importStar(require('../src/api')));
    ...
    test('getProvidedToken falls back to x-api-key and then empty string', async () => {
        const { api } = await loadApiModule();
        expect(api.getProvidedToken({
            header: (name) => (name === 'x-api-key' ? 'key-123' : undefined),
        })).toBe('key-123');
        expect(api.getProvidedToken({
            header: () => undefined,
        })).toBe('');
    ...
    test('authenticateApi returns 401 for invalid token', async () => {
        ...
        const json = jest.fn();
        const next = jest.fn();
        api.authenticateApi(req, { status, json }, next);
    ...
    test('authenticateApi returns 503 when auth token is not configured', async () => {
        ...
        const status = jest.fn().mockReturnThis();
        const json = jest.fn();
        api.authenticateApi({ header: () => undefined }, { status, json }, jest.fn());
    ...
    test('auditApiAccess uses x-admin-user when provided', async () => {
        const { api, logOperation } = await loadApiModule();
        await api.auditApiAccess('api_action', {
            header: (name) => (name === 'x-admin-user' ? 'owner-7' : undefined),
            method: 'PATCH',
            path: '/api/owner',
        });
        expect(logOperation).toHaveBeenCalledWith(expect.objectContaining({ userId: 'owner-7', details: 'PATCH /api/owner' }));
    });
    test('auditApiAccess swallows logging failures', async () => {
        const { api } = await loadApiModule({
            logOperation: jest.fn().mockRejectedValue(new Error('audit failed')),
        });
        await expect(api.auditApiAccess('api_action', {
            header: () => 'root',
            method: 'POST',
            path: '/api/test',
    ...
    test('leaderboard route returns rows for authorized request', async () => {
        ...
        expect(execute).toHaveBeenCalledWith('SELECT user, score FROM leaderboard ORDER BY score DESC LIMIT ?', [7]);
    ...
    test('game-results route uses filtered query for valid game', async () => {
        ...
        expect(execute).toHaveBeenCalledWith('SELECT id, user, game, result, score, created_at FROM game_results WHERE game = ? ORDER BY created_at DESC LIMIT ?', ['dice', 3]);
    });
    test('game-results route uses unfiltered query when game is omitted', async () => {
        ...
        expect(execute).toHaveBeenCalledWith('SELECT id, user, game, result, score, created_at FROM game_results ORDER BY created_at DESC LIMIT ?', [100]);
    });
    ...
    test('affiliates route returns filtered rows for valid status', async () => {
        ...
        expect(execute).toHaveBeenCalledWith('SELECT user_id, status, requested_at, approved_at, approved_by FROM affiliates WHERE status = ? ORDER BY requested_at DESC', ['active']);
    });
    test('affiliates route uses unfiltered query when status is omitted', async () => {
        ...
        expect(execute).toHaveBeenCalledWith('SELECT user_id, status, requested_at, approved_at, approved_by FROM affiliates ORDER BY requested_at DESC');
    });
    ...
    test('codes and audit-log routes return rows with clamped limits', async () => {
        ...
        expect(execute).toHaveBeenNthCalledWith(1, 'SELECT code, source, created_at FROM codes ORDER BY created_at DESC LIMIT ?', [500]);
        expect(execute).toHaveBeenNthCalledWith(2, 'SELECT id, user_id, server_id, action, details, created_at FROM audit_logs ORDER BY created_at DESC LIMIT ?', [1]);
    });
    ...
    test('startApiServer listens on provided port', async () => {
        const { api } = await loadApiModule();
        const listen = jest.spyOn(api.app, 'listen').mockImplementation(((port, callback) => {
            callback?.();
