...
const games_1 = require("../src/games");
function randomSequence(values, fallback = 0.5) {
    let index = 0;
    return () => values[index++] ?? fallback;
}
function cardIndexToRandom(cardIndex) {
    return (cardIndex + 0.01) / 13;
}
describe('Games', () => {
    test('game functions use default random source when omitted', () => {
        const randomSpy = jest.spyOn(Math, 'random').mockReturnValue(0.5);
        const blackjack = (0, games_1.playBlackjack)();
        const plinko = (0, games_1.playPlinko)();
        const mines = (0, games_1.playMines)();
    ...
    test('blackjack includes player, dealer, and result lines', () => {
        const result = (0, games_1.playBlackjack)(() => 0.5);
    ...
    test('blackjack draws additional cards for low totals', () => {
        ...
        const result = (0, games_1.playBlackjack)(randomSequence(values));
    ...
    test('blackjack includes every rank symbol when drawn', () => {
        ...
        const result = (0, games_1.playBlackjack)(randomSequence(values, 0.99));
    ...
    test('blackjack does not hit when player total is exactly 16', () => {
        ...
        const result = (0, games_1.playBlackjack)(randomSequence(values, cardIndexToRandom(12)));
    ...
    test('blackjack keeps player bust when multiple aces and high cards are drawn', () => {
        ...
        const result = (0, games_1.playBlackjack)(randomSequence(values, cardIndexToRandom(1)));
        expect(result).toContain('Player: A A A K K (23)');
        expect(result).toContain('Result: **Dealer wins**');
    });
    test('plinko reports a path, lane, and payout', () => {
        const result = (0, games_1.playPlinko)(() => 0.5);
    ...
    test('plinko stays within lane bounds for extreme left and right paths', () => {
        ...
        const leftResult = (0, games_1.playPlinko)(randomSequence(leftValues, 0.01));
        expect(leftResult).toContain('Landing slot: 1');
        const rightValues = [0.99, 0.99, 0.99, 0.99, 0.99, 0.99];
        const rightResult = (0, games_1.playPlinko)(randomSequence(rightValues, 0.99));
        expect(rightResult).toContain('Landing slot: 11');
    });
    test('plinko uses exactly six steps and maps 0.5 to right', () => {
        const result = (0, games_1.playPlinko)(randomSequence([0.5, 0.5, 0.5, 0.5, 0.5, 0.5], 0.5));
    ...
    test('plinko mixed path validates starting position math', () => {
        const result = (0, games_1.playPlinko)(randomSequence([0.1, 0.9, 0.9, 0.9, 0.9, 0.9], 0.9));
    ...
    test('mines returns a bounded pick and a result', () => {
        ...
        const result = (0, games_1.playMines)(randomSequence(values));
    ...
    test('mines does not hang when random repeats', () => {
        const result = (0, games_1.playMines)(() => 0.5);
        expect(result).toContain('Mines');
        expect(result).toMatch(/Pick: tile [1-9]/);
    });
    test('mines safe path includes multiplier', () => {
        const values = [0.0, 0.12, 0.24, 0.95];
        const result = (0, games_1.playMines)(randomSequence(values, 0.95));
    ...
    test('mines boom path includes boom result', () => {
        ...
        const result = (0, games_1.playMines)(randomSequence(values, 0.05));
    ...
    test('mines fallback placement still completes with repeated random values', () => {
        const result = (0, games_1.playMines)(randomSequence([0, 0, 0, 0], 0));
        expect(result).toContain('Pick: tile 1');
        expect(result).toContain('Result: **Boom**');
    });
    test('mines repeated random uses exactly 101 random calls total', () => {
        let calls = 0;
        const result = (0, games_1.playMines)(() => {
            calls += 1;
            return 0;
        });
        expect(result).toContain('Pick: tile 1');
    ...
    test('mines fallback adds expected mine tiles for tile 2 pick', () => {
        ...
        values.push(0.12);
        const result = (0, games_1.playMines)(randomSequence(values, 0));
