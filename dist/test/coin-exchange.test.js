...
const coin_exchange_1 = require("../src/coin-exchange");
...
describe('coin exchange deep tests', () => {
    ...
    test('parseCoinAmount validates and rounds input', () => {
        expect((0, coin_exchange_1.parseCoinAmount)('abc')).toBeNull();
        expect((0, coin_exchange_1.parseCoinAmount)('-1')).toBeNull();
        expect((0, coin_exchange_1.parseCoinAmount)('0')).toBeNull();
        expect((0, coin_exchange_1.parseCoinAmount)('1.239')).toBe(1.24);
    });
    test('getCoinWallet ensures wallet, maps numeric values, and closes connection', async () => {
        ...
        const wallet = await (0, coin_exchange_1.getCoinWallet)('u1');
        ...
        expect(execute).toHaveBeenNthCalledWith(1, 'INSERT IGNORE INTO coin_wallets (user_id) VALUES (?)', ['u1']);
        expect(execute).toHaveBeenNthCalledWith(2, 'SELECT user_id, available_balance, locked_balance FROM coin_wallets WHERE user_id = ?', ['u1']);
        expect(end).toHaveBeenCalledTimes(1);
    });
    test('transferCoins rejects self-transfer before opening db connection', async () => {
        await expect((0, coin_exchange_1.transferCoins)({ fromUserId: 'same', toUserId: 'same', amount: 5, serverId: 'guild-1' })).rejects.toThrow('You cannot transfer coins to yourself.');
        expect(db_1.getDbConnection).not.toHaveBeenCalled();
    });
    test('transferCoins rolls back when sender has insufficient funds', async () => {
        ...
        const end = jest.fn().mockResolvedValue(undefined);
        const execute = jest.fn().mockImplementation((sql, params) => {
        ...
        db_1.getDbConnection.mockResolvedValue({
            beginTransaction,
            execute,
            rollback,
            commit,
        ...
        await expect((0, coin_exchange_1.transferCoins)({ fromUserId: 'sender', toUserId: 'receiver', amount: 5, serverId: 'guild-1' })).rejects.toThrow('Insufficient available balance.');
    ...
    test('transferCoins updates balances, writes transactions, commits, and audits', async () => {
        ...
        await (0, coin_exchange_1.transferCoins)({ fromUserId: 'sender', toUserId: 'receiver', amount: 4.5, serverId: 'guild-9' });
        expect(commit).toHaveBeenCalledTimes(1);
        expect(rollback).not.toHaveBeenCalled();
        expect(end).toHaveBeenCalledTimes(1);
        expect(execute).toHaveBeenCalledWith('UPDATE coin_wallets SET available_balance = ? WHERE user_id = ?', [5.5, 'guild-9:sender']);
        expect(execute).toHaveBeenCalledWith('UPDATE coin_wallets SET available_balance = ? WHERE user_id = ?', [7.5, 'guild-9:receiver']);
    ...
    test('listCoinOffers uses status-filtered and unfiltered query branches', async () => {
        ...
        const end = jest.fn().mockResolvedValue(undefined);
        db_1.getDbConnection.mockResolvedValue({ execute, end });
        const filtered = await (0, coin_exchange_1.listCoinOffers)(20, 'open');
        const unfiltered = await (0, coin_exchange_1.listCoinOffers)(10, '');
        expect(filtered[0]).toEqual(expect.objectContaining({ id: 1, senderUserId: 'a', recipientUserId: 'b', amount: 1.2, status: 'open' }));
        expect(unfiltered[0]).toEqual(expect.objectContaining({ id: 2, senderUserId: 'x', recipientUserId: 'y', amount: 2, status: 'accepted' }));
        expect(execute).toHaveBeenNthCalledWith(1, 'SELECT * FROM coin_exchange_offers WHERE status = ? ORDER BY created_at DESC LIMIT ?', ['open', 20]);
        expect(execute).toHaveBeenNthCalledWith(2, 'SELECT * FROM coin_exchange_offers ORDER BY created_at DESC LIMIT ?', [10]);
    ...
    test('listCoinTransactions and listCoinWallets map values and enforce query branches', async () => {
        ...
        const end = jest.fn().mockResolvedValue(undefined);
        db_1.getDbConnection.mockResolvedValue({ execute, end });
        const byUser = await (0, coin_exchange_1.listCoinTransactions)(5, 'u1');
        const all = await (0, coin_exchange_1.listCoinTransactions)(2, '');
        const wallets = await (0, coin_exchange_1.listCoinWallets)(1);
        expect(byUser[0]).toEqual(expect.objectContaining({ userId: 'u1', amount: 9.99, balanceAfter: 10.01, kind: 'transfer' }));
        expect(all[0]).toEqual(expect.objectContaining({ userId: 'u3', amount: 3, offerId: 4, direction: 'debit' }));
        expect(wallets[0]).toEqual(expect.objectContaining({ userId: 'wx', availableBalance: 3.33, lockedBalance: 1.67, totalBalance: 5 }));
    ...
    test('grantCoins rolls back and rethrows when update fails', async () => {
        ...
        await expect((0, coin_exchange_1.grantCoins)({
            actorUserId: 'admin',
            targetUserId: 'target',
            amount: 1,
            serverId: 'guild',
    ...
    test('grantCoins stores null details when optional details are omitted', async () => {
        ...
        await (0, coin_exchange_1.grantCoins)({
            actorUserId: 'admin',
            targetUserId: 'target',
            amount: 1,
            serverId: 'guild',
        });
        expect(execute).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO coin_exchange_transactions'), expect.arrayContaining(['guild:target', 'guild:admin', 'credit', 'grant', 1, 2, null, null]));
    });
    test('createExchangeOffer validates sender and recipient are different', async () => {
        await expect((0, coin_exchange_1.createExchangeOffer)({
            senderUserId: 'u1',
            recipientUserId: 'u1',
            amount: 1,
            serverId: 'guild',
    ...
    test('createExchangeOffer uses null note and default transaction details when note is omitted', async () => {
        ...
        const commit = jest.fn().mockResolvedValue(undefined);
        const end = jest.fn().mockResolvedValue(undefined);
        const execute = jest.fn().mockImplementation((sql) => {
            if (sql === 'INSERT IGNORE INTO coin_wallets (user_id) VALUES (?)')
                return Promise.resolve([{}, []]);
            if (sql.includes('FROM coin_wallets WHERE user_id = ? FOR UPDATE')) {
        ...
        await (0, coin_exchange_1.createExchangeOffer)({
            senderUserId: 'sender',
            recipientUserId: 'recipient',
            amount: 2,
            serverId: 'guild',
        });
        expect(execute).toHaveBeenCalledWith('INSERT INTO coin_exchange_offers (sender_user_id, recipient_user_id, amount, note) VALUES (?, ?, ?, ?)', ['guild:sender', 'guild:recipient', 2, null]);
        expect(execute).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO coin_exchange_transactions'), expect.arrayContaining(['guild:sender', 'guild:recipient', 'debit', 'offer_lock', 2, 8, 88, 'Offer created']));
    });
    test('acceptExchangeOffer rejects non-open or wrong-recipient offers', async () => {
        ...
        db_1.getDbConnection.mockResolvedValueOnce({
            beginTransaction,
        ...
        await expect((0, coin_exchange_1.acceptExchangeOffer)({ recipientUserId: 'r', offerId: 5, serverId: 'guild' })).rejects.toThrow('That offer is not open anymore.');
        ...
        await expect((0, coin_exchange_1.acceptExchangeOffer)({ recipientUserId: 'r', offerId: 6, serverId: 'guild' })).rejects.toThrow('You are not the recipient for that offer.');
        expect(commit).not.toHaveBeenCalled();
    ...
    test('cancelExchangeOffer rejects non-sender cancellation attempts', async () => {
        ...
        await expect((0, coin_exchange_1.cancelExchangeOffer)({ requesterUserId: 'intruder', offerId: 8, serverId: 'guild' })).rejects.toThrow('Only the sender can cancel that offer.');
        expect(commit).not.toHaveBeenCalled();
        expect(rollback).toHaveBeenCalledTimes(1);
        expect(audit_log_1.logOperation).not.toHaveBeenCalled();
    });
    test('listUserOffers and listUserTransactions map rows', async () => {
        ...
        const end = jest.fn().mockResolvedValue(undefined);
        db_1.getDbConnection.mockResolvedValue({ execute, end });
        const offers = await (0, coin_exchange_1.listUserOffers)('u1');
        const txs = await (0, coin_exchange_1.listUserTransactions)('u1', 7);
        expect(offers[0]).toEqual(expect.objectContaining({ id: 2, senderUserId: 'u1', recipientUserId: 'u2', amount: 5.5 }));
        expect(txs[0]).toEqual(expect.objectContaining({ id: 3, userId: 'u1', counterpartyUserId: 'u2', balanceAfter: 8 }));
        expect(end).toHaveBeenCalledTimes(2);
    });
    test('createExchangeOffer rolls back when available balance is insufficient', async () => {
        ...
        await expect((0, coin_exchange_1.createExchangeOffer)({ senderUserId: 'sender', recipientUserId: 'recipient', amount: 5, serverId: 'guild' })).rejects.toThrow('Insufficient available balance to create that offer.');
        expect(rollback).toHaveBeenCalledTimes(1);
        expect(commit).not.toHaveBeenCalled();
    });
    test('acceptExchangeOffer rejects when sender locked balance is insufficient', async () => {
        ...
        await expect((0, coin_exchange_1.acceptExchangeOffer)({ recipientUserId: 'recipient', offerId: 11, serverId: 'guild' })).rejects.toThrow('The sender no longer has enough locked balance for this offer.');
        expect(rollback).toHaveBeenCalledTimes(1);
        expect(commit).not.toHaveBeenCalled();
    });
    test('acceptExchangeOffer completes success path and writes audit log', async () => {
        ...
        await (0, coin_exchange_1.acceptExchangeOffer)({ recipientUserId: 'recipient', offerId: 12, serverId: 'guild' });
        expect(execute).toHaveBeenCalledWith('UPDATE coin_wallets SET locked_balance = ? WHERE user_id = ?', [0, 'guild:sender']);
        expect(execute).toHaveBeenCalledWith('UPDATE coin_wallets SET available_balance = ? WHERE user_id = ?', [5, 'guild:recipient']);
        expect(commit).toHaveBeenCalledTimes(1);
        expect(rollback).not.toHaveBeenCalled();
        expect(audit_log_1.logOperation).toHaveBeenCalledWith({
            userId: 'recipient',
    ...
    test('acceptExchangeOffer uses fallback note when offer note is null', async () => {
        ...
        const execute = jest.fn().mockImplementation((sql, params) => {
            ...
            if (sql.includes('FROM coin_wallets WHERE user_id = ? FOR UPDATE')) {
                const userId = params[0];
                if (userId === 'sender' || userId === 'guild:sender')
                    return Promise.resolve([[{ user_id: 'sender', available_balance: 5, locked_balance: 2 }], []]);
                return Promise.resolve([[{ user_id: 'recipient', available_balance: 0, locked_balance: 0 }], []]);
            }
            return Promise.resolve([{}, []]);
        });
        db_1.getDbConnection.mockResolvedValue({ beginTransaction, execute, rollback, commit, end });
        await (0, coin_exchange_1.acceptExchangeOffer)({ recipientUserId: 'recipient', offerId: 13, serverId: 'guild' });
        expect(execute).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO coin_exchange_transactions'), expect.arrayContaining(['guild:recipient', 'guild:sender', 'credit', 'offer_accept', 2, 2, 13, 'Offer accepted']));
    });
    test('cancelExchangeOffer rejects when offer is not open', async () => {
        ...
        await expect((0, coin_exchange_1.cancelExchangeOffer)({ requesterUserId: 'owner', offerId: 20, serverId: 'guild' })).rejects.toThrow('That offer is not open anymore.');
        expect(rollback).toHaveBeenCalledTimes(1);
        expect(commit).not.toHaveBeenCalled();
    });
    test('cancelExchangeOffer completes success path and writes audit log', async () => {
        ...
        await (0, coin_exchange_1.cancelExchangeOffer)({ requesterUserId: 'owner', offerId: 21, serverId: 'guild' });
        expect(execute).toHaveBeenCalledWith('UPDATE coin_wallets SET available_balance = ?, locked_balance = ? WHERE user_id = ?', [3.5, 0, 'owner']);
        expect(commit).toHaveBeenCalledTimes(1);
        expect(rollback).not.toHaveBeenCalled();
        expect(audit_log_1.logOperation).toHaveBeenCalledWith({
            userId: 'owner',
    ...
    test('listUserTransactions uses default limit when omitted', async () => {
        ...
        const end = jest.fn().mockResolvedValue(undefined);
        db_1.getDbConnection.mockResolvedValue({ execute, end });
        const txs = await (0, coin_exchange_1.listUserTransactions)('u1');
        expect(execute).toHaveBeenCalledWith(expect.stringContaining('FROM coin_exchange_transactions'), ['u1', 10]);
    ...
    test('listCoinWallets, listCoinOffers, and listCoinTransactions use defaults when omitted', async () => {
        ...
        const end = jest.fn().mockResolvedValue(undefined);
        db_1.getDbConnection.mockResolvedValue({ execute, end });
        await (0, coin_exchange_1.listCoinWallets)();
        await (0, coin_exchange_1.listCoinOffers)();
        await (0, coin_exchange_1.listCoinTransactions)();
        expect(execute).toHaveBeenNthCalledWith(1, expect.stringContaining('FROM coin_wallets'), [50]);
        expect(execute).toHaveBeenNthCalledWith(2, 'SELECT * FROM coin_exchange_offers ORDER BY created_at DESC LIMIT ?', [50]);
        expect(execute).toHaveBeenNthCalledWith(3, expect.stringContaining('FROM coin_exchange_transactions'), [100]);
        expect(end).toHaveBeenCalledTimes(3);
    });
});
